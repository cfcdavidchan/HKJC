...
with open('game_model.json') as data_file:
...
def backtesting(kelly=False, win_place= False, place_only=False, hkjc_odd_ignore=10, hkjc_ignoare_first= False, place_multiplier= 3, account_money= 25000, game_invest_base= 1000):
    ...
    for race in games_model: #loop over each race
        ...
        if len(may_buy) > 0: # get some horses to buy
            ...
            target_horse_2 = None
            if len(may_buy) > 1:
                may_buy.remove(target_horse)
                target_horse_2 = (min(may_buy, key=lambda x: x[0]))
                reasonable_odd_2 = float(target_horse_2[0])
                hkjc_odd_2 = float(target_horse_2[1])
                premium_2 = float(target_horse_2[2])
                final_place_2 = target_horse_2[3]
                date_2 = target_horse_2[4]
                race_2 = target_horse_2[5]
                horse_number_2 = target_horse_2[6]
                place_odd_2 = target_horse_2[7]
            ...
            if account_money <10: #buy hourse only if the account has more than 10 dollars
                ...
                if win_place: #buy both win and place
                    ...
                    if target_horse_2!= None:
                        place_invest = (win_invest/2) * place_multiplier
                        place_invest = place_invest - place_invest%10
                    else:
                ...
                if place_only:
                    win_invest = 0
                    if target_horse_2 != None:
                        place_invest = (game_invest/2) - (game_invest/2) % 10 #make it can be divide by 10
                    else:
                ...
                if final_place in [1, '1']:
                    win_P_L = win_P_L + (win_invest * (hkjc_odd -1))
                else:
                    win_P_L = win_P_L - win_invest

                # P_L from place
                place_P_L = 0
                if final_place in [1,2,3,'1','2','3']:
                    place_P_L = place_P_L + (place_invest * (place_odd - 1))
                else:
                    place_P_L = place_P_L - place_invest

                if target_horse_2 != None:
                    if final_place_2 in [1,2,3,'1','2','3']:
                        place_P_L = place_P_L + (place_invest * (place_odd_2 - 1))
                    else:
                        place_P_L = place_P_L - place_invest
                ...
                record = [date, race, horse_number, win_invest, hkjc_odd, hkjc_odd_2, win_P_L, place_invest, place_odd, place_odd_2, place_P_L, total_invest, race_P_L, orginial_account_money, account_money]
